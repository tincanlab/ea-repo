...
EFFECTIVE_DA_WORKSTREAM_CATALOG=""
...
ensure_git_workdir_repo() {
  ...
  local default_branch="${OPENARCHITECT_GIT_DEFAULT_BRANCH:-main}"
  local gitignore_path="$workdir/.gitignore"
  mkdir -p "$workdir"
  touch "$gitignore_path"
  if ! grep -Fxq ".opencode/" "$gitignore_path"; then
    printf "%s\n" ".opencode/" >> "$gitignore_path"
    echo "Updated gitignore:"
    echo "  - $gitignore_path (added .opencode/)"
  fi
  ...
  if ! git -C "$workdir" init -b "$default_branch" >/dev/null 2>&1; then
    git -C "$workdir" init >/dev/null
    git -C "$workdir" branch -M "$default_branch" >/dev/null 2>&1 || true
  fi
...
resolve_selector_catalog_path() {
  local role="$1"
  local selector_catalog="${OPENARCHITECT_SELECTOR_CATALOG:-}"
  local sa_catalog_default="architecture/portfolio/initiatives.yml"
  local da_catalog="${OPENARCHITECT_DA_WORKSTREAM_CATALOG:-architecture/solution/domain-workstreams.yml}"
  ...
  if [ "$role" = "da" ]; then
    EFFECTIVE_SELECTOR_CATALOG=""
    if [ -n "$selector_catalog" ]; then
      catalog_rel="$selector_catalog"
    else
      catalog_rel="$da_catalog"
    fi

    if [[ "$catalog_rel" = /* ]]; then
      EFFECTIVE_DA_WORKSTREAM_CATALOG="$catalog_rel"
      return 0
    fi

    if [ -z "${EFFECTIVE_SELECTOR_SOURCE_REPO_URL:-}" ]; then
      echo "ERROR: DA routing requires OPENARCHITECT_SA_REPO_URL or OPENARCHITECT_SELECTOR_SOURCE_REPO_URL when OPENARCHITECT_DA_WORKSTREAM_CATALOG/OPENARCHITECT_SELECTOR_CATALOG is not an absolute mounted path." >&2
      exit 1
    fi
  else
    case "$role" in
      sa) catalog_rel="${selector_catalog:-$sa_catalog_default}" ;;
      *) catalog_rel="$selector_catalog" ;;
    esac
  ...
  if [ "$role" = "da" ]; then
    EFFECTIVE_DA_WORKSTREAM_CATALOG="$fetch_dir/$catalog_rel"
    if [ ! -f "$EFFECTIVE_DA_WORKSTREAM_CATALOG" ]; then
      echo "ERROR: DA Workstream selector '$catalog_rel' not found in selector source repo ${EFFECTIVE_SELECTOR_SOURCE_REPO_URL}" >&2
      exit 1
    fi
    return 0
  fi
...
if [ "${ROUTING_ROLE:-}" = "ea" ]; then
  ...
  if [ "${ROUTING_ROLE}" = "sa" ] || [ "${ROUTING_ROLE}" = "da" ]; then
    ...
    if [ "${ROUTING_ROLE}" = "da" ] && [ -n "${EFFECTIVE_DA_WORKSTREAM_CATALOG:-}" ]; then
      RESOLVE_CMD+=(--da-workstream-catalog "$EFFECTIVE_DA_WORKSTREAM_CATALOG")
    fi
  ...
  echo "  - selector_id=${OPENARCHITECT_SELECTOR_ID:-${INITIATIVE_ID:-${WORKSTREAM_ID:-${WORK_ITEM_ID:-${API_ID:-}}}}}"
  ...
  if [ -n "${EFFECTIVE_DA_WORKSTREAM_CATALOG:-}" ]; then
    echo "  - da_workstream_catalog=${EFFECTIVE_DA_WORKSTREAM_CATALOG}"
  fi

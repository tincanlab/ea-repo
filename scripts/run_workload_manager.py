    ...
    for initiative in initiatives:
        ...
        for workstream in workstreams:
            ...
            if workstream_id:
                rows.append(
                    {
                        "selector_key": f"workstream:{workstream_id}",
                        "kind": "workstream",
                        "role": "da",
                        "selector_id": workstream_id,
                        "status": _normalize_status(workstream.get("status")),
                        "repo_url": str(
                            workstream.get("workstream_repo_url") or ""
                        ).strip(),
...
def _collect_running_docker_compose_projects() -> tuple[dict[str, str], str]:
    cmd = [
        "docker",
        "ps",
        "--format",
    ...
    running_by_project: dict[str, str] = {}
    for line in completed.stdout.splitlines():
        text = str(line).strip()
        if not text:
            continue
        name, _, project = text.partition("|")
        project_name = project.strip()
        if not project_name:
            continue
        if project_name not in running_by_project:
            running_by_project[project_name] = name.strip()
    return running_by_project, ""


def _sync_containers_with_docker(
    conn: sqlite3.Connection,
    *,
    observed_at: str,
) -> dict[str, Any]:
    running_by_project, error = _collect_running_docker_compose_projects()
    probe: dict[str, Any] = {
        "source": "docker_cli",
        "ok": not bool(error),
        "error": error,
        "running_projects": len(running_by_project),
        "running_containers": len(running_by_project),
        "synced_selectors": 0,
        "matched_running": 0,
        "project_keys": sorted(running_by_project.keys()),
    }
    if error:
        return probe

    queue_rows = _query_latest_queue_rows(conn)
    def _resolve_running_match(
        compose_project: str, selector_id: str, role: str
    ) -> tuple[str, str]:
        direct = running_by_project.get(compose_project, "")
        if direct:
            return compose_project, direct

        normalized_target = compose_project.replace("-auto-", "-")
        for running_project, running_name in running_by_project.items():
            normalized_running = running_project.replace("-auto-", "-")
            if normalized_running == normalized_target:
                return running_project, running_name

        token = selector_id.strip().lower()
        role_token = role.strip().lower()
        if token:
            for running_project, running_name in running_by_project.items():
                project_text = running_project.lower()
                name_text = running_name.lower()
                if token not in project_text and token not in name_text:
                    continue
                if role_token and role_token not in project_text and role_token not in name_text:
                    continue
                return running_project, running_name
        return "", ""

    for row in queue_rows:
        compose_project = str(row.get("compose_project") or "").strip()
        ...
        matched_project, matched_name = _resolve_running_match(
            compose_project, selector_id, role
        )
        is_running = bool(matched_project)
        if is_running:
            probe["matched_running"] = int(probe["matched_running"]) + 1
        runtime_state = "running" if is_running else "not_running"
        conn.execute(
            """
            INSERT INTO containers (
              selector_key, role, selector_id, compose_project, container_name, host_port,
            ...
            ON CONFLICT(selector_key) DO UPDATE SET
              ...
              runtime_state = excluded.runtime_state,
              started_at = CASE
                WHEN excluded.runtime_state = 'running'
                  THEN COALESCE(containers.started_at, excluded.started_at)
                ELSE containers.started_at
              END,
              updated_at = excluded.updated_at
            """,
            (
                ...
                compose_project,
                matched_name,
                int(row.get("host_port") or 0)
                if row.get("host_port") is not None
                else None,
                runtime_state,
                observed_at if is_running else None,
                observed_at,
                None,
                "",
            ),
        )
        probe["synced_selectors"] = int(probe["synced_selectors"]) + 1
    return probe


def _build_status_payload(
    conn: sqlite3.Connection, *, runtime_probe: dict[str, Any] | None = None
) -> dict[str, Any]:
    queue_rows = _query_latest_queue_rows(conn)
...
def _push_touches_catalogs(payload: dict[str, Any]) -> bool:
    changed: set[str] = set()
    for commit in payload.get("commits") or []:
        ...
        for key in ("added", "removed", "modified"):
            ...
            if isinstance(paths, list):
                changed.update(str(p) for p in paths)
    if not changed:
        return True
    return any(
        catalog in changed or any(f.endswith("/" + catalog.split("/")[-1]) for f in changed)
        for catalog in _CATALOG_PATHS
    )
...
class _WebhookHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *log_args: Any) -> None:
        print(f"webhook: {format % log_args}")
    ...
    def do_POST(self) -> None:
        ...
        if not _verify_github_signature(body, sig, self.webhook_secret):
        ...
        if event_type == "ping":
            ...
            return
        if event_type != "push":
            self._respond(200, {"message": f"ignored event: {event_type}"})
            return
        ...
        if _push_touches_catalogs(payload):
            ref = str(payload.get("ref") or "")
            print(f"webhook: triggering cycle from push to {ref}")
            self.trigger_event.set()
            self._respond(200, {"message": "triggered"})
        else:
            self._respond(200, {"message": "no catalog changes, skipped"})
...
def _start_webhook_server(
    bind: str, port: int, secret: str, trigger_event: threading.Event
) -> HTTPServer:
    handler = type(
        "_BoundWebhookHandler",
        (_WebhookHandler,),
        {"trigger_event": trigger_event, "webhook_secret": secret},
    )
    server = HTTPServer((bind, port), handler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    print(f"webhook_listener=http://{bind}:{port}/webhook/github")
    return server
...
def _process_once(args: argparse.Namespace) -> int:
    ...
    input_report_json = str(args.input_report_json or "").strip()
    if input_report_json:
        json_report_path = Path(input_report_json).resolve()
        if not json_report_path.exists():
            print(
                f"input report JSON not found: {json_report_path}",
                file=sys.stderr,
            )
            return 2
        returncode = 0
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        runtime_probe = _sync_containers_with_docker(conn, observed_at=_utc_now())
        status_payload = _build_status_payload(conn, runtime_probe=runtime_probe)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Scheduled workload manager: run discovery, persist launch state in SQLite, "
    ...
    parser.add_argument(
        "--ea-repo-url",
        default="",
        help="EA repo URL for discovery entrypoint (required unless --input-report-json is used).",
    )
    parser.add_argument(
        "--input-report-json",
        default="",
        help=(
            "Use an existing discovery JSON report and skip discovery/GitHub fetch for this run."
        ),
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--webhook-listen",
        default="",
        help="Start a GitHub webhook receiver on [bind:]port (e.g. '9090' or '0.0.0.0:9090').",
    )
    parser.add_argument(
        "--webhook-secret",
        default="",
        help="Shared secret for validating GitHub webhook signatures (X-Hub-Signature-256).",
    )
    ...
    if not str(args.input_report_json or "").strip() and not str(
        args.ea_repo_url or ""
    ).strip():
        default_input_report = (
            Path(args.workdir).resolve() / "enterprise-repo-graph.json"
        )
        if default_input_report.exists():
            args.input_report_json = str(default_input_report)
            print(f"input_report_json={default_input_report}")
        else:
            parser.error(
                "--ea-repo-url is required unless --input-report-json is provided "
                f"(default not found: {default_input_report})"
            )

    mode_map = {"report_only": "report_only", "plan": "plan", "launch": "launch"}
    args.mode = mode_map.get(args.mode, "report_only")

    webhook_listen = str(args.webhook_listen or "").strip()
    if not args.poll and not webhook_listen:
        return _process_once(args)

    trigger_event = threading.Event()
    webhook_server = None
    if webhook_listen:
        if ":" in webhook_listen and not webhook_listen.startswith("["):
            bind, _, port_str = webhook_listen.rpartition(":")
        else:
            bind, port_str = "0.0.0.0", webhook_listen
        webhook_server = _start_webhook_server(
            bind, int(port_str), str(args.webhook_secret or ""), trigger_event
        )
    ...
    try:
        while True:
            ...
            if webhook_server and not args.poll:
                trigger_event.wait()
                trigger_event.clear()
            elif webhook_server and args.poll:
                triggered = trigger_event.wait(timeout=interval)
                if triggered:
                    trigger_event.clear()
            else:
                time.sleep(interval)

...
KNOWN_ARTIFACTS = [
    ...
    "architecture/solution/domain-engagements.yml",
...
def _validate_compose_files(compose_root: Path) -> dict[str, Any]:
    ...
    for file_path in files:
        ...
        if role == "ea":
            ...
            for key in ("ENGAGEMENT_ID", "OPENARCHITECT_SA_REPO_URL"):
                if not str(env_map.get(key) or "").strip():
    ...
    *,
    ...
    for initiative in initiatives:
        ...
        for engagement in engagements:
            ...
            engagement_status = _normalize_status(engagement.get("status"))
            domain_repo_url = _normalize_repo_url(
                str(engagement.get("domain_repo_url") or "").strip()
            )
            if engagement_id:
                launch_env = {"ENGAGEMENT_ID": engagement_id}
                if solution_repo_url:
                    launch_env["OPENARCHITECT_SA_REPO_URL"] = solution_repo_url
                entries[f"engagement:{engagement_id}"] = {
                    "key": f"engagement:{engagement_id}",
                    "kind": "engagement",
            ...
            for job in jobs:
                ...
                if domain_repo_url:
                    launch_env["OPENARCHITECT_GIT_REPO_URL"] = domain_repo_url
...
def _to_markdown(report: dict[str, Any]) -> str:
    ...
    lines.append(f"- Engagements discovered: `{summary['engagements']}`")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="SA engagements catalog path (repo-relative).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Default implementation catalog path in domain repos (repo-relative).",
    )
    parser.add_argument(
        "--workdir",
        default=str(
    ...
    for row in initiatives_rows:
        ...
        engagements_path = solution_path / args.engagements_catalog
        if not engagements_path.exists():
            validation_warnings.append(
                f"{source_key}: missing engagements catalog {engagements_path}"
            )
            continue

        try:
            engagements_payload = _load_yaml(engagements_path)
        except ValueError as exc:
            validation_errors.append(str(exc))
            continue
        engagements_rows = engagements_payload.get("engagements")
        if not isinstance(engagements_rows, list):
            validation_errors.append(
                f"{engagements_path}: expected top-level 'engagements' list"
            )
            continue

        for engagement in engagements_rows:
            if not isinstance(engagement, dict):
                continue
            engagements_total += 1
            engagement_id = str(engagement.get("engagement_id") or "").strip()
            engagement_status = _normalize_status(engagement.get("status"))
            domain_repo_url = str(engagement.get("domain_repo_url") or "").strip()
            engagement_initiative_id = str(
                engagement.get("initiative_id") or ""
            ).strip()
            domain_id = str(engagement.get("domain_id") or "").strip()
            handoff_ref = str(engagement.get("handoff_ref") or "").strip()
            engagement_item: dict[str, Any] = {
                "engagement_id": engagement_id,
                "status": engagement_status,
                "initiative_id": engagement_initiative_id,
                "domain_id": domain_id,
                "domain_repo_url": _normalize_repo_url(domain_repo_url),
                "handoff_ref": handoff_ref,
                "jobs": [],
            }
            initiative_item["engagements"].append(engagement_item)
            engagement_source = (
                f"{source_key}/engagement:{engagement_id or '<missing-id>'}"
            )

            if not engagement_id:
                validation_errors.append(f"{engagement_source}: missing engagement_id")
            if (
                engagement_initiative_id
                and initiative_id
                and engagement_initiative_id != initiative_id
            ):
                validation_errors.append(
                    f"{engagement_source}: initiative_id mismatch ({engagement_initiative_id} != {initiative_id})"
                )
            if not domain_id:
                validation_errors.append(f"{engagement_source}: missing domain_id")
            elif registry_loaded and domain_id not in registry_domain_ids:
                validation_errors.append(
                    f"{engagement_source}: domain_id '{domain_id}' is not defined in {args.domain_registry_catalog}"
                )
            if not domain_repo_url:
                validation_errors.append(
                    f"{engagement_source}: missing domain_repo_url"
                )
                continue
            if not _status_is_active(engagement_status, args.allow_inactive):
                validation_warnings.append(
                    f"{engagement_source}: skipped inactive engagement (status={engagement_status})"
                )
                continue

            domain_path = ensure_repo(domain_repo_url, "domain", engagement_source)
            if not domain_path:
                continue

            implementation_catalog_rel = str(
                engagement.get("implementation_catalog_path")
                or engagement.get("implementation_catalog")
                or engagement.get("implementation_catalog_file")
                or args.implementation_catalog
            ).strip()
            implementation_catalog_path = domain_path / implementation_catalog_rel
            if not implementation_catalog_path.exists():
                validation_warnings.append(
                    f"{engagement_source}: implementation catalog not found: {implementation_catalog_path}"
                )
                continue

            try:
                design_payload = _load_json(implementation_catalog_path)
            except ValueError as exc:
                validation_errors.append(str(exc))
                continue
            jobs = design_payload.get("implementation_work_items")
            if not isinstance(jobs, list):
                validation_warnings.append(
                    f"{engagement_source}: no implementation_work_items in {implementation_catalog_path}"
                )
                continue
            ...
            for job in jobs:
                ...
                engagement_item["jobs"].append(job_item)
                job_source = f"{engagement_source}/job:{selector}"

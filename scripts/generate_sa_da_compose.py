...
def build_da_compose(WORKSTREAM_ID: str, sa_repo_url: str) -> str:
    return (
        "services:\n"
        "  opencode:\n"
        "    environment:\n"
...
def main() -> int:
    ...
    workstream_default = f"ws-{initiative_id}-om"
    WORKSTREAM_ID = prompt_validated(
        "DA WORKSTREAM_ID",
        default=workstream_default,
        validator=lambda raw: validate_id(
            raw, label="WORKSTREAM_ID", required_prefix="ws-"
        ),
    )
    ...
    print(f"  DA WORKSTREAM_ID: {WORKSTREAM_ID}")
    ...
    write_file(DA_OUT, build_da_compose(WORKSTREAM_ID, sa_repo_url))
    return 0
